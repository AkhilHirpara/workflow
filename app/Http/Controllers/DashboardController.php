...
class DashboardController extends Controller
{
    ...
    public function dashboarddetails(Request $request){
        ...
        if ($check_user->role == 2) {
            ...
            $getdata = $getdata->whereNotIn('status', [0,2]);
        } else {
            // For admin users
        ...
        foreach ($result['projects_list'] as $key => $pdata) {
            ...
            if ($totalimportdata->count() > 0) {
                $data = $totalimportdata->first(); // Get the first result
                // Add integrity_statustotal to the current project
                $result['projects_list'][$key]->integrity_statustotal = (int) $totalimportdata->integrity_statustotal;;
                $result['projects_list'][$key]->task_statustotal = (int) $totalimportdata->task_statustotal;
            } 
