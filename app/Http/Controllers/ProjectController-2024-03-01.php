    ...
    public function addeditproject(Request $request)
    {
        ...
        if (empty($is_exist)) {
            ...
            if ($request->has('projectid') && trim($request->projectid) != '') {
                ...
                $addeditdata->update(['project_name' => $request->project_name, 'identifier' => $request->identifier, 'investor_id' => $request->investor_id, 'platform_id' => $request->platform_id, 'status' => $status, 'file_completeness' => $filecompletness]);
                ...
                $addeditdata = Project::create(['project_name' => $request->project_name, 'identifier' => $request->identifier, 'investor_id' => $request->investor_id, 'platform_id' => $request->platform_id, 'percentage_completed' => 0, 'integrity_precentage_completed' => 0, 'status' => 3, 'last_completed_step' => 2, 'created_by' => $check_user->id, 'file_completeness' => $filecompletness, 'only_integrity' => $only_integrity, 'only_review' => $only_review]);
